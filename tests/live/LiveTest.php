...
class LiveTest extends TestCase
{
    ...
    {
        if (is_null($this->ripe)) {
            $adapter = new \Test\Guzzle7Adapter($options);
            $this->ripe = new WebService($adapter);
        }
        return $this->ripe;
    }
    ...
    public function testCreateInetnum()
    {
        ...
        $prefix = '127.0.' . date('H') . '.';
        $firstIp = $prefix . date('i');
        $secondIp = $prefix . date('i') + 1;

        $ip = new Inetnum($firstIp . ' - ' . $secondIp);
